/* 🎬 SATURN STUDIO CYBER FORTRESS (500% HARDENED) - COPYRIGHT © 2026 MOHAMED HALLALI */
/* STRICTLY PROPRIETARY • UNAUTHORIZED DUPLICATION, SCRAPING, OR AI TRAINING IS PROHIBITED BY LAW */
!(function(){
  'use strict';
  // Anti-tamper & self-defending integrity check
  var _0xdef = function(){
    var _0xch = function(){
      var _0xr = new RegExp('^function\\s*\\(\\s*\\)');
      return !_0xr.test(_0xdef.toString());
    };
    if (_0xch()) {
...
const emailVal = (window.SITE_DATA && window.SITE_DATA.contact && window.SITE_DATA.contact.email) ? window.SITE_DATA.contact.email : "hallali.mohamed4@gmail.com";
...
try {
console.log(
'%c🎬 SATURN STUDIO | MOHAMED HALLALI %c\n%c🔒 هذا البورتفوليو محمي بأنظمة أمان وتشفير متقدمة 500%%.\n© 2026 جميع الحقوق محفوظة لـ Mohamed Hallali.\nجميع الأكواد، الفيديوهات، والوسائط محمية بموجب القوانين الدولية لحماية الملكية الفكرية.\nStrictly Proprietary • Unauthorized Duplication, Reverse-Engineering, or AI Scraping is Prohibited by International Law.',
...
e.preventDefault();
e.stopPropagation();
showToast(currentLang === 'ar' ? '🔒 جميع الأكواد، التصاميم، والمحتوى محمية بموجب حقوق الملكية الفكرية الدولية' : '🔒 All source code, design, and assets are protected by international copyright laws');
return false;
}
}, true);
...
const isIllegalCopyOrSelect = !isEditable && (
((e.ctrlKey || e.metaKey) && ['A', 'a', 'C', 'c', 'X', 'x'].includes(e.key))
);
if (isF12 || isDevTools || isSaveOrSource || isIllegalCopyOrSelect) {
e.preventDefault();
e.stopPropagation();
showToast(currentLang === 'ar' ? '🛡️ الكود المصدري ومحتويات الموقع محمية ضد النسخ والتطفل والسرقة' : '🛡️ Source code and assets are protected against copying and inspection');
return false;
}
}, true);
...
? '⚠️ [تحذير قانوني رسمي]\nهذا البورتفوليو والتصميم البرمجي محمي دولياً بموجب حقوق الملكية الفكرية لصالح Mohamed Hallali (SATURN STUDIO) © 2026.\nيُمنع منعاً باتاً استنساخ الكود أو تغذية الذكاء الاصطناعي به لإنشاء أعمال مشتقة.'
: '⚠️ [OFFICIAL LEGAL NOTICE]\nThis portfolio architecture and source code is strictly proprietary and copyrighted by Mohamed Hallali (SATURN STUDIO) © 2026.\nUnauthorized duplication, scraping, or feeding to AI models is strictly prohibited by international law.';
if (e.clipboardData) {
e.clipboardData.setData('text/plain', warningText);
}
showToast(currentLang === 'ar' ? '⚠️ النسخ محظور لحماية حقوق الملكية الفكرية' : '⚠️ Copying is prohibited to protect intellectual property');
}
});
let devToolsShield = document.getElementById('devtools-shield-overlay');
if (!devToolsShield) {
devToolsShield = document.createElement('div');
devToolsShield.id = 'devtools-shield-overlay';
devToolsShield.className = 'devtools-shield-overlay';
devToolsShield.innerHTML = `
<div class="devtools-shield-card">
<div class="devtools-shield-icon">
<i class="ri-shield-keyhole-fill"></i>
</div>
<h3 class="devtools-shield-title">${currentLang === 'ar' ? 'أدوات المطورين محظورة' : 'DevTools Access Prohibited'}</h3>
<p class="devtools-shield-desc">
${currentLang === 'ar'
? 'تم إغلاق العرض لحماية الشفرة البرمجية والملكية الفكرية للموقع ضد النسخ وسرقة الأكواد. يرجى إغلاق أدوات المطورين للمتابعة.'
: 'Access is restricted to protect intellectual property and source code against theft. Please close Developer Tools to continue.'}
</p>
<button type="button" class="devtools-shield-btn" id="devtools-shield-close">
${currentLang === 'ar' ? 'إعادة تحميل الصفحة الآمنة' : 'Reload Secure Page'}
</button>
</div>
`;
document.body.appendChild(devToolsShield);
const closeBtn = document.getElementById('devtools-shield-close');
if (closeBtn) {
...
if (isOpen) {
devToolsShield.classList.add('active');
try {
(function() {
Function('debugger')();
})();
...
devToolsShield.classList.remove('active');
}
};
